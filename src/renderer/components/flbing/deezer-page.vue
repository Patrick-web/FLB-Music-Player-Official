<template>
  <div class="deezerResults">
    ...
    <div
      v-if="searchResults.tracks.length"
      class="results trackResults"
    >
      <h2>Tracks</h2>
  ...
  export default {
    ...
    methods: {
      ...
      searchInDeezer(query) {
        ...
        fetch(
          `https://api.deezer.com/search?q=artist:"${query}"`,
          requestOptions
        )
          .then(response => response.text())
          .then(result => {
        ...
        fetch(
          `https://api.deezer.com/search?q=album:"${query}"`,
          requestOptions
        )
          .then(response => response.text())
          .then(result => {
