...
import { AlbumType, ArtistType, FolderInfoType, FolderParsedType, PlaylistType, TrackType } from "../../types";
...
const mutations = {
    ...
    addSelectedTracksToPlaylist(state: any, payload: string) {
        state.tabsData.playlists.forEach((playlist: PlaylistType, index: number) => {
            if (playlist.name === payload) {
                state.selectedTracks.forEach((track: TrackType) => {
...
const actions: ActionTree<any, any> = {
    ...
    fetchArtistInfo({ state, commit }) {
        fetch(
            ...
            .then((result) => {
                console.log(JSON.parse(result).results.slice(0, 1));
            })
