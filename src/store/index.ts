...
import { removeDuplicates, sendMessageToNode, sortArrayOfObjects } from "../Utils/frontEndUtils";
...
const store: StoreOptions<RootStateType> = {
  ...
  mutations: {
    ...
    setRenderedTrack(state: RootStateType, payload: Array<TrackType>) {
      state.renderedTracks = payload;
    },
    addToSelectedTracks: (state: RootStateType, payload: TrackType) => {
      const index = state.selectedTracks.findIndex(track=>track.fileLocation==payload.fileLocation)
      if(index>-1){
    ...
    setPlayingTrack: (state: RootStateType, payload: TrackType) => {
      ...
      if (
        ...
        state.tabsData.recentTracks = removeDuplicates(state.tabsData.recentTracks,'fileLocation')
      }
      //Check if its the Favourites
      document.querySelector(".playingPane")?.classList.remove("favored");
      state.tabsData.playlists[0].tracks.forEach(track=>{
    ...
    search(state: RootStateType, query: string) {
      if (query) {
        state.searchResults.tracks = state.tabsData.addedTracks
          .filter(
            (track) =>
              track.defaultTitle?.toLowerCase()?.includes(query.toLocaleLowerCase()) ||
              track.fileName?.toLowerCase()?.includes(query.toLocaleLowerCase()) ||
              track.artist?.toLowerCase()?.includes(query.toLocaleLowerCase())||
              track.extractedArtist?.toLowerCase()?.includes(query.toLocaleLowerCase()) 
          )
          .slice(0, 4);
    ...
    generateFoldersData: (state: RootStateType) => {
      ...
      folders.forEach(folder=>{
        ...
        const tracks = state.tabsData.addedTracks.filter((track)=>track.folderInfo.path===folder.path)
      ...
      state.tabsData.playlists.forEach((playlist,index)=>{
        if(playlist.name===payload){
          state.selectedTracks.forEach(track=>{
            ...
            state.tabsData.playlists[index].tracks = removeDuplicates(state.tabsData.playlists[index].tracks,'fileLocation')
          })
        }
      })
      sendMessageToNode('updatePlaylists',state.tabsData.playlists)
    ...
    deleteSelectedTrackToPlaylist(state: RootStateType){
      state.tabsData.playlists.forEach((playlist,pindex)=>{
        if(playlist.name===state.selectedGroup?.name){
          state.selectedTracks.forEach((track,tindex)=>{
  ...
  actions: {
    determineNextTrack({ commit,dispatch, state }, direction: string) {
      ...
      if(state.audioState.shuffle){
        const randomIndex = Math.ceil(Math.random()*state.renderedTracks.length)
    ...
    removeSelectedTracks({commit,state}){
      state.selectedTracks.forEach(track=>{
        const index1 = state.tabsData.addedTracks.findIndex(trackk=>trackk.fileLocation==track.fileLocation)
        const index2 = state.renderedTracks.findIndex(trackk=>trackk.fileLocation==track.fileLocation)
    ...
    getLyrics({state},track: TrackType){
      ...
      const options = {
        apiKey: 'OvIJKwGiXVXykOca4Yr5V4ys0wReNjxazasnInKUqGItUzgih9uQjrYK6YmSSzeS',
      ...
      getSong(options).then((song: geniusSongType) =>{
        if(song){
          ...
          const lyricsTabIcon: HTMLElement = document.getElementsByClassName('lyricsTabIcon')[0] as HTMLElement;
          lyricsTabIcon.click()
        }
      }
);
